...
from PyQt5.QtWidgets import (
    ...
    QWidgetAction ,QOpenGLWidget,QStackedLayout,QCheckBox
)
...
class ROSListener(Node):
    def __init__(self):
        ...
        self.led_pub = self.create_publisher(BoolMsg, '/led1', 10)

        self.chat_switch_pub = self.create_publisher(Bool, '/chat_switch', 10)
...
class MainWindow(QMainWindow):
    def __init__(self, ros_node):
        ...
        with open('/home/user/ros2_uboot/src/task_scheduler/task_scheduler/room.pos', 'r') as file:
        ...
        self.setStyleSheet("""
            ...
            QPushButton { font: 24pt 'Segoe UI'; }
        """)
        ...
        for lbl in (self.temp_label, self.humi_label, self.time_label):
            lbl.setStyleSheet("color: white; font: 20pt 'Segoe UI'; padding: 0 10px;")
        ...
        nav_label.setStyleSheet("font-size: 30pt;") 
        ...
        # ---- 地图部分 ----
        map_label = QLabel()
        map_pix = QPixmap("hotel.png").scaled(700, 700, Qt.KeepAspectRatio, Qt.SmoothTransformation)
        ...
        nav_normal  = "background-color:#4CAF50; color:white; border-radius:8px;font-size: 24pt;"
        nav_pressed = "background-color:#45a049; color:white; border-radius:8px;font-size: 24pt;"
        for b in (btn1, btn2):
        ...
        g_label.setStyleSheet("font-size: 30pt;") 
        ...
        d_label.setStyleSheet("font-size: 30pt;") 
        ...
        chat_layout.setContentsMargins(130, 0, 0, 0)
        chat_layout.setSpacing(15)
        chat_layout.setAlignment(Qt.AlignLeft)

        # 左侧 GIF
        self.chat_gif = QLabel()
        self.chat_gif.setFixedSize(500, 500)
        ...
        interact_normal = "background-color:#3498db; color:white; border-radius:10px;font-size: 24pt;"
        interact_pressed = "background-color:#2980b9; color:white; border-radius:10px;font-size: 24pt;"
        ...
        # ====== 话题切换滑动按钮 ====== #
        # 创建语音控制按钮 (滑动按钮)
        voice_control_btn = QCheckBox("")
        voice_control_btn.setFixedSize(160, 80)  # 按钮整体尺寸

        voice_control_btn.setStyleSheet("""
            QCheckBox {
        ...
        # 创建一个容器，使用 QVBoxLayout 将按钮排列
        btn_container = QWidget()
        btn_layout = QHBoxLayout(btn_container)  # 使用 QHBoxLayout 横向排列
        btn_layout.setAlignment(Qt.AlignCenter)  # 水平居中排列按钮
        btn_layout.setSpacing(80)  # 设置按钮间距
        btn_layout.setContentsMargins(0, 0, 0, 0)  # 设置容器的内边距，保持一定距离

        # 按钮加入横向布局
        btn_layout.addWidget(voice_control_btn)  # 加入语音控制按钮
        btn_layout.addWidget(on_btn)  # 加入开灯按钮
        btn_layout.addWidget(off_btn)  # 加入关灯按钮
        ...
        on_btn.clicked.connect(self.on_light_on)
        off_btn.clicked.connect(self.on_light_off)
        voice_control_btn.stateChanged.connect(lambda state: self.on_voice_control(state))  # 添加语音控制的回调
        ...
        c_layout.setContentsMargins(0, 0, 0, 10)
    ...
    def on_voice_control(self, state):
        """
        处理语音控制按钮的状态变化。
        如果按钮被选中，则发布 True；否则发布 False 到 /chat_switch。
        """
        msg = Bool()
        msg.data = (state == Qt.Checked)   # True/False
        self.ros.chat_switch_pub.publish(msg)
        print(f"语音控制 {'开启' if msg.data else '关闭'} ")
    ...
    def on_goto_room1(self):
        ...
        self.ros.chat_pub.publish(String(data="1"))
    ...
    def on_goto_room2(self):
        ...
        # 假设房间102坐标 (4.0, 1.0)
        self.ros.chat_pub.publish(String(data="3"))
